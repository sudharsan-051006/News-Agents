...
const DriftWall = ({
  items = [], // Starts empty instead of showing nature images by default
  columns = 5,
  tileWidth = 200,
  tileHeight = 132,
  gap = 18,
  ...
  // Safe items check (renders empty if database items are still loading)
  const safeItems = useMemo(() => {
    return Array.isArray(items) && items.length > 0 ? items : [];
  }, [items]);

  const columnItems = useMemo(() => {
    if (safeItems.length === 0) return Array.from({ length: columns }, () => []);
    ...
    >
      <div ref={planeRef} className="drift-wall__plane">
        {columnItems.map((col, c) => {
          const meta = columnMeta[c];
          if (!meta || col.length === 0) return null;
